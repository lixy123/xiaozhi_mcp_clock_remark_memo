...
{
  baidu_access_token = getAccessToken(baidu_api_key.c_str(), baidu_secret_key.c_str());
}
...
void playAudio(uint8_t* audioData, size_t audioDataSize) {
  if (audioDataSize > 0) {
    ...
    I2S_Write(audioData, audioDataSize);
  }
}
...
void baiduTTS_Send(String text) {
  ...
  Serial.println("baiduTTS_Send:" + text);
  const int per = 0;   //音库，度小宇=1 男，度小美=0，度逍遥（基础）=3 男，度丫丫=4 男童
  const int spd = 5;   //语速，取值0-15，默认为5中语速
  const int pit = 5;   //音调，取值0-15，默认为5中语调
  const int vol = 10;  //音量. 取值0-9
  const int aue = 6;   //4为pcm-16k；5为pcm-8k；6为wav 16k16位带文件头 pcm不带wav头
  ...
  url += "&vol=" + String(vol);
  url += "&aue=" + String(aue);
  url += "&cuid=esp32s3";   //用户唯一标识，用来计算UV值。建议填写能区分用户的机器 MAC 地址或 IMEI 码，长度为60字符以内
  url += "&lan=zh";  //中文
  url += "&ctp=1";
...
void playAudio_Zai(void) {
  ...
  //RAM‌：520KB，用于程序变量和运行时数据，实际用户可用DRAM约260–300KB。‌‌
  //50KB
